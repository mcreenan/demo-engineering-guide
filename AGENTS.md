# AGENTS.md — Demo Engineering Guide
...
- The generated artifact (`demo-engineering-guide.html`) is **overwritten on every build.** Editing
...
- After any change: **`pal build`** reassembles the artifact.
...
- **Repo names are links, never monospace.** Write a Git repo as
  ...
  `https://github.com/demo-co/REPO` with a GitHub icon. Repo-root links only — **never**
...
<!-- pal:sources:start -->
#### Engineering Wiki (wiki)

Canonical home for architecture decisions (ADRs), runbooks, and process.
...
The backend API monorepo (Node + TypeScript, Postgres) — authoritative for how the system behaves.

Authoritative for how the system actually behaves (the code is the source of truth).
- Local checkout: `sources/code/platform` — grep/read it to verify behavior.
- Freshness: confirm the clone is current (`git -C sources/code/platform log -1 --format=%cr`) before trusting an absence.
- Link in the guide as <a class="gh-repo" data-repo="platform"></a>; deep references resolve under https://github.com/demo-co/platform. Repo-root links only.

- Local path: `sources/code/platform`
- Links: https://github.com/demo-co/platform

#### web (repo)

The frontend monorepo (Next.js apps + shared packages) — authoritative for what the product looks like.

Authoritative for how the system actually behaves (the code is the source of truth).
- Local checkout: `sources/code/web` — grep/read it to verify behavior.
- Freshness: confirm the clone is current (`git -C sources/code/web log -1 --format=%cr`) before trusting an absence.
- Link in the guide as <a class="gh-repo" data-repo="web"></a>; deep references resolve under https://github.com/demo-co/web. Repo-root links only.

- Local path: `sources/code/web`
- Links: https://github.com/demo-co/web

#### infra (repo)

Core infrastructure (Terraform, Kubernetes, CI/CD) — authoritative for how the system is run.

Authoritative for how the system actually behaves (the code is the source of truth).
- Local checkout: `sources/code/infra` — grep/read it to verify behavior.
- Freshness: confirm the clone is current (`git -C sources/code/infra log -1 --format=%cr`) before trusting an absence.
- Link in the guide as <a class="gh-repo" data-repo="infra"></a>; deep references resolve under https://github.com/demo-co/infra. Repo-root links only.

- Local path: `sources/code/infra`
- Links: https://github.com/demo-co/infra

#### Help Center (help)

Customer-facing help articles (an Astro content repo).

Authoritative for product-facing behavior (help/marketing content).
- Local checkout: `sources/docs/help-center` — search the article source (e.g. Astro/Markdown content).
- Cite the published article URL under https://help.demo.co.
- On conflict: update the help article first, then the guide.

- Local path: `sources/docs/help-center`
- Links: https://help.demo.co
<!-- pal:sources:end -->
...
named `sections/<n>-<slug>.html` fragment, **(3)** rebuild with `pal build`.
